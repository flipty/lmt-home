...
<label class="screen-reader-text" for="s">' . __('',  'habanero') . '</label>
<input type="search" value="' . get_search_query() . '" name="s" id="s" placeholder="Search..." />
<input type="submit" id="searchsubmit" value="'. esc_attr__('Search', 'habanero') .'" />
</form>';
return $form;
}
...
function numeric_posts_nav() {
    ...
    if ( get_previous_posts_link() )
        printf( '<li class="prev">%s</li>' . "\n", get_previous_posts_link( __( '<', 'habanero' ) ) );
    ...
    if ( get_next_posts_link() )
        printf( '<li class="next">%s</li>' . "\n", get_next_posts_link( __( '>', 'habanero' )) );
...
function wpex_get_excerpt( $args = array() ) {
	...
	$defaults = array(
		...
		'length'          => 40,
		'readmore'        => false,
		'readmore_text'   => esc_html__( 'read more', 'habanero' ),
		'readmore_after'  => '',
